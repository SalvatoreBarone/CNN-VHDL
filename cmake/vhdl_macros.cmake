...
macro (add_testbench_source)
    ...
    foreach (_src ${ARGV0})
        ...
        add_custom_target("${ARGV1}" COMMAND ${CMAKE_VHDL_COMPILER} -a --ieee=synopsys -v "${CMAKE_SOURCE_DIR}/${FILE_SRC}" &&
                                                 ${CMAKE_VHDL_COMPILER} -e --ieee=synopsys -v "${ARGV1}" WORKING_DIRECTORY ${CMAKE_BINARY_DIR})
        add_custom_target("${TEST_NAME}" COMMAND ${CMAKE_VHDL_COMPILER} -r --ieee=synopsys -v ${ARGV1}  --stop-time=1ms --vcd=${TRACE_VCD_PATH} --fst=${TRACE_FST_PATH} WORKING_DIRECTORY ${CMAKE_BINARY_DIR} DEPENDS "${ARGV1}")

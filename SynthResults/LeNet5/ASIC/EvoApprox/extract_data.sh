...
for i in {0..10}; do
  ...
  do
    ...
    conf=${data_width}_bit_${vol_depth}_${vol_height}_${vol_width}_approx_${approx}
    ...
    leak=$(grep -e "evo_neuron " ${name} | sed "s/ \+/,/g" | cut -d ',' -f3);
    dyna=$(grep -e "evo_neuron " ${name} | sed "s/ \+/,/g" | cut -d ',' -f4);
    tota=$(grep -e "evo_neuron " ${name} | sed "s/ \+/,/g" | cut -d ',' -f5);
